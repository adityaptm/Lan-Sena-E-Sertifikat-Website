...
export default function Dashboard() {
  ...
  // FIX: Export ke Excel (.xlsx) murni agar angka 0 di depan aman
  const downloadExcel = () => {
    const excelData = data.map((item, index) => ({
      No: index + 1,
      Blok: item.blok,
      "No SHGB": String(item.nomor_shgb || ""), // Paksa jadi string
      Desa: item.desa,
      Luas: item.luas,
      Posisi: item.posisi,
      Status: item.proses,
    ...
    // Trik agar Excel tidak memformat angka panjang/nol di depan
    const range = XLSX.utils.decode_range(worksheet["!ref"]!);
    for (let R = range.s.r + 1; R <= range.e.r; ++R) {
      const cellAddress = XLSX.utils.encode_cell({ r: R, c: 2 }); // Kolom 'No SHGB'
  ...
  const handleAction = async (action: string, payload: any) => {
    ...
    try {
      ...
      const res = await fetch("/api/sertifikat", {
        ...
        body: JSON.stringify({
          ...payload,
          nomor_shgb: String(payload.nomor_shgb), // Simpan sebagai string
          action,
          editor: currentUser,
          tanggal: timestamp,
        }),
  ...
  const stats = {
    ...
    persentase:
      data.length > 0
        ? Math.round(
            (data.filter((item) => item.proses === "Selesai").length /
              data.length) *
              100,
          )
        : 0,
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-[#0A3660] text-white">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-black text-xs tracking-tighter uppercase">
            LAN SENA
          </h1>
        </div>
        <button
      ...
      <main className="flex-1 p-4 md:p-10 w-full overflow-x-hidden">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            <button
              ...
              className="flex-1 lg:flex-none px-5 py-3.5 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-sm hover:bg-slate-50 transition-all"
            >
            ...
            <button
              ...
              className="flex-[2] lg:flex-none px-8 py-3.5 bg-[#0A3660] text-white rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-xl hover:bg-[#0d467a] transition-all"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
          {[
            ...
            {
              label: "Proses",
              val: stats.belumSelesai,
              color: "text-amber-600",
              sub: "On Progress",
            },
            {
              label: "Progress",
              val: `${stats.persentase}%`,
              color: "text-white",
              bg: "bg-[#0A3660]",
              sub: "Achievement",
            },
            ...
            <div
              key={i}
              className={`${s.bg || "bg-white"} p-5 md:p-6 rounded-[2rem] border ${s.bg ? "border-transparent" : "border-slate-100"} shadow-sm`}
            >
              <p
                className={`text-[9px] font-black uppercase tracking-widest ${s.bg ? "text-emerald-400" : "text-slate-400"}`}
              >
                {s.label}
              </p>
              ...
              <p
                className={`text-[8px] font-bold uppercase mt-1 ${s.bg ? "text-white/50" : "text-slate-300"}`}
              >
                {s.sub}
              </p>
            </div>
                  ...
                  .map((item) => (
                    <tr key={item.id} className="hover:bg-blue-50/30 group">
                      ...
                      <td className="px-8 py-5">
                        <div
                          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl ${posisiConfig[item.posisi]?.bg} ${posisiConfig[item.posisi]?.text}`}
                        >
                          <i
                            className={`bx ${posisiConfig[item.posisi]?.icon} text-sm`}
                          ></i>
                      ...
                      <td className="px-8 py-5">
                        <span
                          className={`text-[10px] font-black uppercase ${item.proses === "Selesai" ? "text-emerald-500" : "text-amber-500"}`}
                        >
                          {item.proses}
                        </span>
                      </td>
          ...
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl p-8 md:p-12 max-h-[90vh] overflow-y-auto">
              ...
              {[
                ...
                <div key={f.k} className="space-y-1.5">
                  ...
                  <input
                    ...
                    // FIX: Paksa uppercase saat input dan simpan
                    onChange={(e) =>
                      setForm({ ...form, [f.k]: e.target.value.toUpperCase() })
                    }
                  />
                </div>
              ))}
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={loading}
                className="flex-[2] py-4 bg-[#0A3660] text-white rounded-[1.5rem] font-black text-[10px] uppercase shadow-xl disabled:opacity-50 hover:bg-[#0d467a] transition-all"
              >
