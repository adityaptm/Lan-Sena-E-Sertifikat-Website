    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <main className="flex-1 p-4 md:p-10 w-full overflow-x-hidden">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          <div>
            <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight uppercase">Halo, {currentUser}!</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            <button onClick={downloadExcel} className="flex-1 lg:flex-none px-5 py-3.5 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-sm">
              <i className="bx bxs-file-export text-emerald-500 text-lg"></i> Export Excel
            </button>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
          {["Semua", "Selesai", "Belum Selesai"].map((filter) => (
            <button
              key={filter}
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-blue-50/30 group">
                      <td className="px-8 py-5 font-black text-[#0A3660] text-sm uppercase">{item.blok}</td>
                      <td className="px-8 py-5 font-mono text-[10px] font-bold text-slate-500 uppercase">{item.nomor_shgb}</td>
                      ...
                      <td className="px-8 py-5">
                        <p className="text-[10px] text-slate-400 italic max-w-[150px] truncate group-hover:whitespace-normal group-hover:text-slate-600 cursor-help uppercase" title={item.keterangan}>{item.keterangan || "—"}</p>
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-[#0A3660] uppercase">{item.editor || "System"}</span>
                          <span className="text-[9px] font-bold text-slate-400 mt-0.5 flex items-center gap-1 uppercase"><i className="bx bx-time-five"></i> {item.tanggal}</span>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[200] p-4">
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl p-8 md:p-12 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black text-[#0A3660] tracking-tight uppercase">{form.id ? "Update" : "Tambah"} <span className="text-emerald-500">Berkas</span></h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
              {[ { l: "Blok Unit", k: "blok" }, { l: "No. SHGB", k: "nomor_shgb" }, { l: "Nama Desa", k: "desa" }, { l: "Luas (m²)", k: "luas" } ].map((f) => (
                <div key={f.k} className="space-y-1.5">
                  ...
                  <input className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl text-sm font-bold focus:bg-white focus:border-[#0A3660]/20 outline-none uppercase" value={(form as any)[f.k]} onChange={(e) => setForm({ ...form, [f.k]: e.target.value.toUpperCase() })} />
                </div>
              ))}
              ...
              <div className="md:col-span-2 space-y-1.5">
                ...
                <textarea className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl text-sm font-bold min-h-[100px] uppercase" placeholder="ISI KETERANGAN..." value={form.keterangan} onChange={(e) => setForm({ ...form, keterangan: e.target.value.toUpperCase() })} />
              </div>
            </div>
